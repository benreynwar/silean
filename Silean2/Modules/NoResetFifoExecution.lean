...
namespace Silean2.Modules.NoResetFifo.Execution

open Silean2.Contracts

abbrev Word (signalType : SignalType) := signalType.Denote
abbrev Input (signalType : SignalType) :=
  Silean2.Contracts.NoResetFifo.Execution.Input (Word signalType)
...
def step (cycleBehavior : CycleBehavior signalType)
    ...
    Silean2.Contracts.NoResetFifo.Execution.StepResult cycleBehavior.state.Values (Word signalType) :=
...
def model (cycleBehavior : CycleBehavior signalType) :
    Silean2.Contracts.NoResetFifo.Execution.Model cycleBehavior.state.Values (Word signalType) where
...
@[simp] theorem step_enqReady (cycleBehavior : CycleBehavior signalType)
    (state : cycleBehavior.state.Values) (input : Input signalType) :
    (step cycleBehavior state input).observation.enqReady =
...
@[simp] theorem step_deqValid (cycleBehavior : CycleBehavior signalType)
    (state : cycleBehavior.state.Values) (input : Input signalType) :
    (step cycleBehavior state input).observation.deqValid =
...
@[simp] theorem step_deqData (cycleBehavior : CycleBehavior signalType)
    (state : cycleBehavior.state.Values) (input : Input signalType) :
    (step cycleBehavior state input).observation.deqData =
...
theorem serial_step_decomposes (upstream downstream : CycleBehavior signalType)
    ...
    Silean2.Contracts.NoResetFifo.Execution.StepDecomposition
...
def serial (upstream downstream : CycleBehavior signalType) :
    Silean2.Contracts.NoResetFifo.Execution.Serial
...
end Silean2.Modules.NoResetFifo.Execution
