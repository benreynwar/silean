...
namespace Silean2.Modules.Fifo.Properties
...
private theorem pointerValue_decompose (addressWidth : Nat)
    ...
    pointerValue addressWidth pointer =
      (if FifoPointerControl.pointerWrap pointer then capacity addressWidth else 0) +
        BitVector.toNat addressWidth
          (FifoPointerControl.pointerAddress pointer) := by
  rfl
...
private theorem entryIndex_eq_pointerAddress (addressWidth : Nat)
  ...
  have bound := BitVector.toNat_lt_cardinality addressWidth
    (FifoPointerControl.pointerAddress pointer)
  rw [BitVector.cardinality_eq_pow] at bound
  cases wrap : FifoPointerControl.pointerWrap pointer <;>
    simp [capacity] <;>
...
theorem occupancy_eq_capacity_iff_full (addressWidth : Nat)
    ...
    occupancy addressWidth readPointer writePointer = capacity addressWidth ↔
      FifoPointerControl.full readPointer writePointer = true := by
  ...
  let writeAddress := BitVector.toNat addressWidth
    (FifoPointerControl.pointerAddress writePointer)
  ...
  rw [FifoPointerControl.full_eq_true_iff, addressEquality]
  unfold occupancy at valid ⊢
  rw [pointerModulus_eq, pointerValue_decompose, pointerValue_decompose]
    at valid ⊢
  cases readWrap : FifoPointerControl.pointerWrap readPointer <;>
    cases writeWrap : FifoPointerControl.pointerWrap writePointer
  all_goals
...
theorem occupancy_eq_zero_iff_empty (addressWidth : Nat)
    (readPointer writePointer : Pointer addressWidth) :
    occupancy addressWidth readPointer writePointer = 0 ↔
      FifoPointerControl.empty readPointer writePointer = true := by
  rw [occupancy_eq_zero_iff,
    FifoPointerControl.empty_eq_true_iff]

theorem contents_eq_nil_iff_empty (addressWidth : Nat)
    (state : ContractState element addressWidth) :
    contents addressWidth state = [] ↔
      FifoPointerControl.empty (state .readPointer) (state .writePointer) = true := by
...
theorem contents_length_eq_capacity_iff_full (addressWidth : Nat)
    ...
    (contents addressWidth state).length = capacity addressWidth ↔
      FifoPointerControl.full (state .readPointer) (state .writePointer) = true := by
  rw [contents_length]
...
private theorem nextEntries_of_acceptedInput (addressWidth : Nat)
  ...
  · have decided : decide (index = BitVector.toIndex addressWidth
        (FifoPointerControl.pointerAddress writePointer)) = false := by
      simp [selected]
    rw [decided]
...
private theorem occupancy_positive_of_readAdvance (addressWidth : Nat)
  ...
  have outputValid :=
    (FifoPointerControl.readAdvance_eq_true_iff readPointer writePointer ready).mp
      accepted |>.1
  ...
  have empty := (occupancy_eq_zero_iff_empty addressWidth readPointer writePointer).mp
    zero
  simp [FifoPointerControl.outputValid, empty] at outputValid
...
private theorem occupancy_lt_capacity_of_writeAdvance (addressWidth : Nat)
  ...
  have inputReady :=
    (FifoPointerControl.writeAdvance_eq_true_iff readPointer writePointer
  ...
  have full := (occupancy_eq_capacity_iff_full addressWidth readPointer writePointer
    stateValid).mp equal
  simp [FifoPointerControl.inputReady, full] at inputReady
...
abbrev Input (element : SignalType) := ResetFifo.Execution.Input (Word element)

def contractInputs (element : SignalType) (input : Input element) :
    (ports element).inputs.Values
  | .inputValid => input.enqValid
  | .inputData => input.enqData
  | .outputReady => input.deqReady
  | .reset => input.reset

def evaluated (element : SignalType) (addressWidth : Nat)
    (state : ContractState element addressWidth) (input : Input element) :=
  (cycleContract element addressWidth).evaluate (contractInputs element input) state

def observedCycle (element : SignalType) (addressWidth : Nat)
    (state : ContractState element addressWidth) (input : Input element) :
    ResetFifo.Cycle (Word element) where
  enqValid := input.enqValid
  enqData := input.enqData
  enqReady := (evaluated element addressWidth state input).1 .inputReady
  deqValid := (evaluated element addressWidth state input).1 .outputValid
  deqData := (evaluated element addressWidth state input).1 .outputData
  deqReady := input.deqReady
  reset := input.reset

def step (element : SignalType) (addressWidth : Nat)
    (state : ContractState element addressWidth) (input : Input element) :
    ResetFifo.Execution.StepResult (ContractState element addressWidth)
      (Word element) where
  nextState := (evaluated element addressWidth state input).2
  observation := observedCycle element addressWidth state input

def model (element : SignalType) (addressWidth : Nat) :
    ResetFifo.Execution.Model (ContractState element addressWidth) (Word element) where
  step := step element addressWidth

theorem evaluated_outputs (element : SignalType) (addressWidth : Nat)
    (state : ContractState element addressWidth) (input : Input element) :
    let outputs := (evaluated element addressWidth state input).1
  ...
  have evaluates := (cycleContract element addressWidth).evaluate_evaluatesTo
    (contractInputs element input) state
...
@[simp] theorem evaluated_next_readPointer (element : SignalType)
    ...
    (input : Input element) :
    (evaluated element addressWidth state input).2 .readPointer =
      nextReadPointer addressWidth input.reset input.deqReady
        (state .readPointer) (state .writePointer) := by rfl

@[simp] theorem evaluated_next_writePointer (element : SignalType)
    (addressWidth : Nat) (state : ContractState element addressWidth)
    (input : Input element) :
    (evaluated element addressWidth state input).2 .writePointer =
      nextWritePointer addressWidth input.reset input.enqValid
        (state .readPointer) (state .writePointer) := by rfl

@[simp] theorem evaluated_next_entries (element : SignalType)
    (addressWidth : Nat) (state : ContractState element addressWidth)
    (input : Input element) :
    (evaluated element addressWidth state input).2 .entries =
      nextEntries addressWidth input.enqValid input.enqData
        (state .readPointer) (state .writePointer) (state .entries) := by rfl

theorem step_preserves_invariant (element : SignalType) (addressWidth : Nat)
    (state : ContractState element addressWidth) (input : Input element)
    (valid : Invariant addressWidth state) :
    Invariant addressWidth (step element addressWidth state input).nextState := by
  change Invariant addressWidth (evaluated element addressWidth state input).2
  unfold Invariant
  change occupancy addressWidth (state .readPointer) (state .writePointer) ≤
    capacity addressWidth at valid
  cases reset : input.reset
  · rw [evaluated_next_readPointer, evaluated_next_writePointer]
    simp only [reset]
    rw [occupancy_next_of_notReset addressWidth (state .readPointer)
      (state .writePointer) input.deqReady input.enqValid valid]
    ...
    · have below := occupancy_lt_capacity_of_writeAdvance addressWidth
        (state .readPointer) (state .writePointer) input.enqValid valid writeAccepted
      simp
      omega
    · simp
      omega
...
theorem step_correct (element : SignalType) (addressWidth : Nat)
    (state : ContractState element addressWidth) (input : Input element)
    (valid : Invariant addressWidth state) :
    ResetFifo.Step (contents addressWidth state)
      (step element addressWidth state input).observation
      (contents addressWidth (step element addressWidth state input).nextState) := by
  rcases evaluated_outputs element addressWidth state input with
    ⟨outputValidEq, outputDataEq, inputReadyEq⟩
  cases reset : input.reset
  · unfold ResetFifo.Step
    simp only [step, observedCycle, reset]
    have raw := contentsOf_next_of_notReset addressWidth (state .entries)
      (state .readPointer) (state .writePointer) input.deqReady input.enqValid
      input.enqData valid
    simpa [contents, ResetFifo.Cycle.acceptedInput,
      ResetFifo.Cycle.acceptedOutput, NoResetFifo.Cycle.acceptedInput,
      NoResetFifo.Cycle.acceptedOutput, observedCycle, step, reset,
      outputValidEq, outputDataEq, inputReadyEq, Fifo.writeAdvance,
      Fifo.readAdvance, Fifo.inputReady, Fifo.outputValid,
      FifoPointerControl.writeAdvance,
      FifoPointerControl.readAdvance] using raw
  · unfold ResetFifo.Step
    simp only [step, observedCycle, reset, if_true]
    unfold contents contentsOf
    change CircularBuffer.values (capacity addressWidth)
      ((evaluated element addressWidth state input).2 .entries)
      (entryIndex addressWidth
        ((evaluated element addressWidth state input).2 .readPointer))
      (occupancy addressWidth
    ...
    simp [reset, nextReadPointer, nextWritePointer,
      EnabledResetCounter.nextValue, occupancy, pointerValue_zeroPointer,
      CircularBuffer.distance]

theorem run_correct (element : SignalType) (addressWidth : Nat)
    (initial : ContractState element addressWidth) (inputs : List (Input element))
    (valid : Invariant addressWidth initial) :
    let result := (model element addressWidth).run initial inputs
    Invariant addressWidth result.finalState ∧
      ResetFifo.Transitions (contents addressWidth initial) result.observations
        (contents addressWidth result.finalState) := by
  exact ResetFifo.Execution.Model.run_correct (model element addressWidth)
    (Invariant addressWidth) (contents addressWidth)
    (step_preserves_invariant element addressWidth)
    (step_correct element addressWidth) initial inputs valid

theorem reset_clears_contents (element : SignalType) (addressWidth : Nat)
    (state : ContractState element addressWidth) (input : Input element)
    (valid : Invariant addressWidth state) (reset : input.reset = true) :
    contents addressWidth (step element addressWidth state input).nextState = [] := by
  exact (step_correct element addressWidth state input valid).reset reset

theorem run_preserves_capacity (element : SignalType) (addressWidth : Nat)
    (initial : ContractState element addressWidth) (inputs : List (Input element))
    (valid : Invariant addressWidth initial) :
    let result := (model element addressWidth).run initial inputs
    (contents addressWidth result.finalState).length ≤ capacity addressWidth := by
  intro result
  exact contents_bounded addressWidth result.finalState
    (run_correct element addressWidth initial inputs valid).1

theorem run_conservation_without_reset (element : SignalType) (addressWidth : Nat)
    (initial : ContractState element addressWidth) (inputs : List (Input element))
    (valid : Invariant addressWidth initial)
    (ordinary : ∀ cycle,
      cycle ∈ ((model element addressWidth).run initial inputs).observations →
        cycle.reset = false) :
    contents addressWidth initial ++
        ResetFifo.acceptedInputs
          ((model element addressWidth).run initial inputs).observations =
      ResetFifo.acceptedOutputs
          ((model element addressWidth).run initial inputs).observations ++
        contents addressWidth
          ((model element addressWidth).run initial inputs).finalState := by
  exact (run_correct element addressWidth initial inputs valid).2
    |>.conservation_without_reset ordinary

theorem run_outputs_preserve_fifo_order_from_empty (element : SignalType)
    (addressWidth : Nat) (initial : ContractState element addressWidth)
    (inputs : List (Input element)) (valid : Invariant addressWidth initial)
    (empty : contents addressWidth initial = [])
    (ordinary : ∀ cycle,
      cycle ∈ ((model element addressWidth).run initial inputs).observations →
        cycle.reset = false) :
    (ResetFifo.acceptedOutputs
      ((model element addressWidth).run initial inputs).observations).IsPrefix
      (ResetFifo.acceptedInputs
        ((model element addressWidth).run initial inputs).observations) := by
  have transitions := (run_correct element addressWidth initial inputs valid).2
  rw [empty] at transitions
  exact transitions.outputs_prefix_inputs_of_empty ordinary

end Execution

end Silean2.Modules.Fifo.Properties
