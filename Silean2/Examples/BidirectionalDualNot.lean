...
namespace Silean2.Examples.BidirectionalDualNot
...
def children : Certified.Children body
  | .a | .b => Examples.ModuleCycleCertified.dualNotCertified
...
def outputRule (input : Examples.DualNot.Input)
    (output : Examples.DualNot.Output) :
...
private theorem certifiedForward
    ...
    (proposal : ProposedValues Examples.HierarchicalDualNot.moduleStructure)
    (satisfies : Examples.HierarchicalDualNot.moduleStructure.IsSolution
      inputs currentState proposal) :
    proposal.outputs .forward = !inputs .forward := by
  let certified := Examples.ModuleCycleCertified.dualNotCertified
  ...
  have holds := evaluates.1 Examples.DualNot.Rule.forward
  change Examples.DualNot.forwardRule.Holds inputs contractState
    proposal.outputs at holds
  simpa [Examples.DualNot.forwardRule, CycleOutputRule.Holds,
...
private theorem certifiedBackward
  ...
  rcases certified.implements inputs contractState currentState proposal
      corresponds satisfies with ⟨nextState, evaluates, nextCorresponds⟩
  have holds := evaluates.1 Examples.DualNot.Rule.backward
  change Examples.DualNot.backwardRule.Holds inputs contractState
    proposal.outputs at holds
  simpa [Examples.DualNot.backwardRule, CycleOutputRule.Holds,
    SignalSelection.project, SignalSelection.Matches, SignalMap.select] using holds

def aInputs (inputs : ports.inputs.Values) :
    Examples.DualNot.ports.inputs.Values
...
theorem implements : Implements moduleStructure cycleContract stateCorresponds := by
  ...
  have outputForward : outputs .forward = inputs .forward := by
    have atBoundary := boundary Examples.DualNot.Output.forward
  ...
  have outputBackward : outputs .backward = inputs .backward := by
    have atBoundary := boundary Examples.DualNot.Output.backward
