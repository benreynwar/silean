...
namespace Silean2.Examples.Checks.ResetContract
...
def contract : Silean2.ModuleResetContract ports where
  State := List Bool
  resetInput := .reset
  resetInputType := rfl
  resetState := []
...
example (inputs : List ports.inputs.Values) (outputs : List ports.outputs.Values)
  ...
  Silean2.ModuleResetContract.TraceMatches.before_first_reset contract
    lengths ordinary

end Silean2.Examples.Checks.ResetContract
