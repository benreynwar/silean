...
namespace PicoRV.Datapath.LoadRs2Update.Description.Internal

open Silean Silean.Naming Silean.Authoring.CircuitDescription

private theorem same : some description = ofNaming LoadRs2Update.naming := by
  simp only [circuit_description, description, construction]
  simp [circuit_description, enumeration]
  unfold LoadRs2Update.moduleStructure LoadRs2Update.naming
  simp only [id_eq]
  rw [ofNaming_composite]
  unfold ofCompositeNaming portList
  simp only [enumeration, List.map_cons, List.map_nil]
  dsimp [LoadRs2Update.body, LoadRs2Update.context,
    LoadRs2Update.instancePorts, LoadRs2Update.structuralChildren,
    LoadRs2Update.wiring, sourceDescription, EndpointContext.moduleInput,
...
theorem corresponds : Corresponds description LoadRs2Update.naming :=
  ⟨same, unique⟩

end PicoRV.Datapath.LoadRs2Update.Description.Internal
