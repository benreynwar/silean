...
module_design LoadRs1Update (name := "picorv32_datapath_load_rs1_update") where
  ...
  instances {
    ...
    lowFiveRs2 (name := .indexed "vector_layout" 0) := Silean.Modules.VectorLayout.design 32 5 LoadRs1Update.lowFiveLayout,
    luiOperand (name := .indexed "mux" 0) := Silean.Modules.Mux.design (.vector 32 .bit),
    selectedOp1 (name := .indexed "mux" 1) := Silean.Modules.Mux.design (.vector 32 .bit),
    trapOp1 (name := .indexed "mux" 2) := Silean.Modules.Mux.design (.vector 32 .bit),
    op2Immediate (name := .indexed "mux" 3) := Silean.Modules.Mux.design (.vector 32 .bit),
    op2Shift (name := .indexed "mux" 4) := Silean.Modules.Mux.design (.vector 32 .bit),
    op2Load (name := .indexed "mux" 5) := Silean.Modules.Mux.design (.vector 32 .bit),
    op2Lui (name := .indexed "mux" 6) := Silean.Modules.Mux.design (.vector 32 .bit),
    trapOp2 (name := .indexed "mux" 7) := Silean.Modules.Mux.design (.vector 32 .bit),
    shiftImmediate (name := .indexed "mux" 8) := Silean.Modules.Mux.design (.vector 5 .bit),
    shiftLoad (name := .indexed "mux" 9) := Silean.Modules.Mux.design (.vector 5 .bit),
    shiftLui (name := .indexed "mux" 10) := Silean.Modules.Mux.design (.vector 5 .bit),
    trapShift (name := .indexed "mux" 11) := Silean.Modules.Mux.design (.vector 5 .bit),
    finalShift (name := .indexed "mux" 12) := Silean.Modules.Mux.design (.vector 5 .bit),
