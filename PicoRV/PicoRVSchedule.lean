...
noncomputable def allRulesSchedule :
    Silean.Contracts.Cycle.Certification.Layer.Schedule body childContracts (fun _ => True) (Silean.Contracts.Cycle.Certification.Layer.CoversAllRules body childContracts) [] :=
  derivedCompleteSchedule.schedule.replaceFinish
    derivedCompleteSchedule.coversAllRules

noncomputable def stateSchedule : Silean.Contracts.Cycle.Certification.Layer.StateSchedule body childContracts :=
  derivedCompleteSchedule.schedule

abbrev ParentOutputSchedule (output : Output) :=
  Silean.Contracts.Cycle.Certification.Layer.Schedule body childContracts (fun _ => True)
    (fun available => Silean.Contracts.Cycle.Certification.Layer.sourceAvailable (fun _ => True) available
...
noncomputable def outputSchedule (output : Output) : ParentOutputSchedule output :=
  allRulesSchedule.mapFinish fun _available covers =>
    Silean.Contracts.Cycle.Certification.Layer.sourceAvailable_of_covers covers (body.wiring.moduleOutput output)
