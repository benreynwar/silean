...
namespace PicoRV.Decoder.InstructionSummary
...
def bitValue (inputs : Inputs) (output : Output) : Bool :=
  Eq.mp (congrArg Silean.SignalType.Denote
...
@[reducible] private def trapInputs : SignalGroup inputMap :=
  Silean.SignalGroup.fromLabels inputMap TrapRule.Input trapInput

@[reducible] private def trapOutputs : SignalGroup outputMap :=
  Silean.SignalGroup.fromLabels outputMap TrapRule.Output fun
...
@[reducible] private def summaryOutputs : SignalGroup outputMap :=
  Silean.SignalGroup.fromLabels outputMap SummariesRule.Output fun
...
def trapOutputRule : Silean.Contracts.Cycle.CycleOutputRule ports emptySignalMap where
  readsInputs := trapInputs
  writesOutputs := trapOutputs
  target inputs _ := fun
    | .instr_trap => !(recognized (trapValues inputs))

def summariesOutputRule : Silean.Contracts.Cycle.CycleOutputRule ports emptySignalMap where
...
@[simp] theorem trapOutputRule_holds_iff
  ...
  simp only [trapOutputRule, Silean.Contracts.Cycle.CycleOutputRule.Holds,
    Silean.SignalGroup.fromLabels_matches_iff, outputValues]
  constructor
  · intro holds
...
@[simp] theorem summariesOutputRule_holds_iff
  ...
  simp only [summariesOutputRule, Silean.Contracts.Cycle.CycleOutputRule.Holds,
...
end PicoRV.Decoder.InstructionSummary
