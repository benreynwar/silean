...
module_child_certifications childContracts for body where
  inputsValue := Silean.Modules.NamedTupleCombiner.certification
  ...
  writePending := Silean.Primitives.orCertified.certification,
  cpuregsWrite := Silean.Primitives.andCertified.certification
...
module_rule_schedules derivedRuleSchedules for body with childContracts
  ...
  output
    | .outputs => [
        ...
        .writePending => Silean.Primitives.OrRule.apply,
        .cpuregsWrite => Silean.Primitives.AndRule.apply]
  state := [
...
private theorem equalFetchState (bits : Fin 8 → Bool) :
  ...
  have representation : stateBits cpuStateFetch =
      Silean.BitVector.ofNat 8 cpuStateFetch := by
    funext index
...
private theorem implements :
  ...
  have inputsValueInputs : body.wiring.childInputValues hierStep.inputs
    ...
    cases field <;>
      simp [body, wiring, context,
        Silean.EndpointContext.moduleInput, Silean.SignalSource.value, Inputs.toValues,
        inputsOfValues]
  ...
  have writePendingValue : hierStep.childOutputs .writePending .output =
    ...
    have equation := (Silean.Primitives.orOutputRule_holds_iff _ _ _).mp
      (writePendingMatch.ruleHolds Silean.Primitives.OrRule.apply)
    normalize_child_hyp equation unfolding wiring, context
  ...
  have cpuregsWriteValue : hierStep.childOutputs .cpuregsWrite .output =
    ...
    have equation := (Silean.Primitives.andOutputRule_holds_iff _ _ _).mp
      (cpuregsWriteMatch.ruleHolds Silean.Primitives.AndRule.apply)
    normalize_child_hyp equation unfolding wiring, context
