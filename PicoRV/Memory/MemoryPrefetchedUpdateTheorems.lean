...
theorem outputs_of_allowed {step : cycleContract.Step}
    ...
    step.outputs .state =
      stateMap.pack (StateUpdate.outputState (fun inputs _ updated => prefetchedNextState inputs updated) step.inputs) :=
  (StateUpdate.outputRule_holds_iff (fun inputs _ updated => prefetchedNextState inputs updated)
