...
capture definition in `MemoryLookaheadCapture.lean`. -/
module_design LookaheadCapture (name := "picorv32_memory_lookahead_capture") where
  ...
  instances {
    ...
    lookahead (name := .indexed "memory_lookahead" 0) := Lookahead.design,
    active (name := .indexed "or" 0) := Silean.Primitives.orDesign,
    zeroMask (name := .indexed "constant" 0) :=
      Silean.Modules.Constant.design (.vector 4 .bit) (maskOfNat 0),
    writeMask (name := .indexed "mux" 0) :=
      Silean.Modules.Mux.design (.vector 4 .bit),
    address (name := .indexed "mux" 1) :=
      Silean.Modules.Mux.design (.vector 32 .bit),
    mask (name := .indexed "mux" 2) :=
      Silean.Modules.Mux.design (.vector 4 .bit),
    writeData (name := .indexed "mux" 3) :=
      Silean.Modules.Mux.design (.vector 32 .bit),
