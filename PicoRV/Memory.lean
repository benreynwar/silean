...
signal_schema MemoryState where
  mem_state : Silean.Authoring.SignalSchema.vector 2 Silean.Authoring.SignalSchema.bit,
  mem_valid : Silean.Authoring.SignalSchema.bit,
  mem_instr : Silean.Authoring.SignalSchema.bit,
  mem_addr : Silean.Authoring.SignalSchema.vector 32 Silean.Authoring.SignalSchema.bit,
  mem_wdata : Silean.Authoring.SignalSchema.vector 32 Silean.Authoring.SignalSchema.bit,
  mem_wstrb : Silean.Authoring.SignalSchema.vector 4 Silean.Authoring.SignalSchema.bit,
  mem_rdata_q : Silean.Authoring.SignalSchema.vector 32 Silean.Authoring.SignalSchema.bit
...
signal_schema MemoryInputs where
  ...
  next_pc : Silean.Authoring.SignalSchema.vector 32 Silean.Authoring.SignalSchema.bit,
  reg_op1 : Silean.Authoring.SignalSchema.vector 32 Silean.Authoring.SignalSchema.bit,
  reg_op2 : Silean.Authoring.SignalSchema.vector 32 Silean.Authoring.SignalSchema.bit,
  mem_wordsize : Silean.Authoring.SignalSchema.vector 2 Silean.Authoring.SignalSchema.bit,
  mem_ready : Silean.Authoring.SignalSchema.bit,
  mem_rdata : Silean.Authoring.SignalSchema.vector 32 Silean.Authoring.SignalSchema.bit
...
@[simp] theorem stateOfNat_zero : Silean.BitVector.toNat 2 (stateOfNat 0) = 0 := by decide
@[simp] theorem stateOfNat_one : Silean.BitVector.toNat 2 (stateOfNat 1) = 1 := by decide
@[simp] theorem stateOfNat_two : Silean.BitVector.toNat 2 (stateOfNat 2) = 2 := by decide
@[simp] theorem stateOfNat_three : Silean.BitVector.toNat 2 (stateOfNat 3) = 3 := by decide

def wordSize (inputs : Inputs) : Nat := Silean.BitVector.toNat 2 inputs.mem_wordsize
def stateNumber (state : stateMap.Values) : Nat := Silean.BitVector.toNat 2 (state .mem_state)
...
def lowByte (word : Word) : Nat := Silean.BitVector.toNat 32 word % 256
def lowHalf (word : Word) : Nat := Silean.BitVector.toNat 32 word % 65536

def formattedWriteDataFrom (wordsize : TwoBits) (regOp2 : Word) : Word :=
  match Silean.BitVector.toNat 2 wordsize with
...
def formattedWriteMaskFrom (wordsize : TwoBits) (regOp1 : Word) : ByteMask :=
  ...
  | 2 => fun index => decide
      (index.val = Silean.BitVector.toNat 2 (lowAddressBits regOp1))
  | _ => maskOfNat 0xf
...
def formattedReadDataFrom (wordsize : TwoBits) (regOp1 memRdata : Word) : Word :=
  ...
  | 2 =>
      let byte := Silean.BitVector.toNat 2 (lowAddressBits regOp1)
      fun index =>
...
theorem completed_read_or_write_returns_idle (inputs : Inputs)
  ...
  all_goals
    have requestBits : Silean.BitVector.toNat 2 (state .mem_state) = _ := requestState
    have laRead : memLaRead inputs state = false := by
      simp [memLaRead, resetn, requestState]
    have laWrite : memLaWrite inputs state = false := by
      simp [memLaWrite, resetn, requestState]
...
theorem prefetch_transfer_waits_for_instruction_command (inputs : Inputs)
  ...
  have readBits : Silean.BitVector.toNat 2 (state .mem_state) = 1 := readState
...
theorem prefetched_instruction_completes_without_transfer (inputs : Inputs)
  ...
  have prefetchedBits : Silean.BitVector.toNat 2 (state .mem_state) = 3 := prefetched
  have notIdle : stateNumber state ≠ 0 := by omega
  have laRead : memLaRead inputs state = false := by
    simp [memLaRead, resetn, notIdle]
  have laWrite : memLaWrite inputs state = false := by
...
@[reducible] private def registeredOutputs : SignalGroup outputMap :=
  Silean.SignalGroup.fromLabels outputMap RegisteredRule.Output fun
    | .mem_valid => .mem_valid
    | .mem_instr => .mem_instr
    | .mem_addr => .mem_addr
    | .mem_wdata => .mem_wdata
...
@[reducible] private def memLaReadInputs : SignalGroup inputMap :=
  Silean.SignalGroup.fromLabels inputMap MemLaReadRule.Input fun
    | .resetn => .resetn
...
@[reducible] private def memLaReadOutputs : SignalGroup outputMap :=
  Silean.SignalGroup.fromLabels outputMap MemLaReadRule.Output fun
    | .mem_la_read => .mem_la_read

@[reducible] private def memLaWriteInputs : SignalGroup inputMap :=
  Silean.SignalGroup.fromLabels inputMap MemLaWriteRule.Input fun
    | .resetn => .resetn
    | .mem_do_wdata => .mem_do_wdata

@[reducible] private def memLaWriteOutputs : SignalGroup outputMap :=
  Silean.SignalGroup.fromLabels outputMap MemLaWriteRule.Output fun
    | .mem_la_write => .mem_la_write

@[reducible] private def memLaAddrInputs : SignalGroup inputMap :=
  Silean.SignalGroup.fromLabels inputMap MemLaAddrRule.Input fun
    | .mem_do_prefetch => .mem_do_prefetch
    | .mem_do_rinst => .mem_do_rinst
    | .next_pc => .next_pc
    | .reg_op1 => .reg_op1
...
@[reducible] private def memLaAddrOutputs : SignalGroup outputMap :=
  Silean.SignalGroup.fromLabels outputMap MemLaAddrRule.Output fun
    | .mem_la_addr => .mem_la_addr

@[reducible] private def memLaWdataInputs : SignalGroup inputMap :=
  Silean.SignalGroup.fromLabels inputMap MemLaWdataRule.Input fun
    | .mem_wordsize => .mem_wordsize
    | .reg_op2 => .reg_op2

@[reducible] private def memLaWdataOutputs : SignalGroup outputMap :=
  Silean.SignalGroup.fromLabels outputMap MemLaWdataRule.Output fun
    | .mem_la_wdata => .mem_la_wdata

@[reducible] private def memLaWstrbInputs : SignalGroup inputMap :=
  Silean.SignalGroup.fromLabels inputMap MemLaWstrbRule.Input fun
    | .mem_wordsize => .mem_wordsize
    | .reg_op1 => .reg_op1

@[reducible] private def memLaWstrbOutputs : SignalGroup outputMap :=
  Silean.SignalGroup.fromLabels outputMap MemLaWstrbRule.Output fun
    | .mem_la_wstrb => .mem_la_wstrb

@[reducible] private def memDoneInputs : SignalGroup inputMap :=
  Silean.SignalGroup.fromLabels inputMap MemDoneRule.Input fun
    | .resetn => .resetn
    | .mem_do_rinst => .mem_do_rinst
    | .mem_do_rdata => .mem_do_rdata
    | .mem_do_wdata => .mem_do_wdata
...
@[reducible] private def memDoneOutputs : SignalGroup outputMap :=
  Silean.SignalGroup.fromLabels outputMap MemDoneRule.Output fun
    | .mem_done => .mem_done

@[reducible] private def memRdataWordInputs : SignalGroup inputMap :=
  Silean.SignalGroup.fromLabels inputMap MemRdataWordRule.Input fun
    | .mem_wordsize => .mem_wordsize
    | .reg_op1 => .reg_op1
    | .mem_rdata => .mem_rdata

@[reducible] private def memRdataWordOutputs : SignalGroup outputMap :=
  Silean.SignalGroup.fromLabels outputMap MemRdataWordRule.Output fun
    | .mem_rdata_word => .mem_rdata_word

@[reducible] private def memRdataLatchedInputs : SignalGroup inputMap :=
  Silean.SignalGroup.fromLabels inputMap MemRdataLatchedRule.Input fun
    | .mem_ready => .mem_ready
    | .mem_rdata => .mem_rdata

@[reducible] private def memRdataLatchedOutputs : SignalGroup outputMap :=
  Silean.SignalGroup.fromLabels outputMap MemRdataLatchedRule.Output fun
    | .mem_rdata_latched => .mem_rdata_latched

@[reducible] private def memRdataQOutputs : SignalGroup outputMap :=
  Silean.SignalGroup.fromLabels outputMap MemRdataQRule.Output fun
    | .mem_rdata_q => .mem_rdata_q

def registeredRule : Silean.Contracts.Cycle.CycleOutputRule ports stateMap where
...
def memLaReadRule : Silean.Contracts.Cycle.CycleOutputRule ports stateMap where
...
def memLaWriteRule : Silean.Contracts.Cycle.CycleOutputRule ports stateMap where
...
def memLaAddrRule : Silean.Contracts.Cycle.CycleOutputRule ports stateMap where
...
def memLaWdataRule : Silean.Contracts.Cycle.CycleOutputRule ports stateMap where
...
def memLaWstrbRule : Silean.Contracts.Cycle.CycleOutputRule ports stateMap where
...
def memDoneRule : Silean.Contracts.Cycle.CycleOutputRule ports stateMap where
...
def memRdataWordRule : Silean.Contracts.Cycle.CycleOutputRule ports stateMap where
...
def memRdataLatchedRule : Silean.Contracts.Cycle.CycleOutputRule ports stateMap where
...
def memRdataQRule : Silean.Contracts.Cycle.CycleOutputRule ports stateMap where
  readsInputs := .empty inputMap
...
@[simp] theorem registeredRule_holds_iff (inputs : inputMap.Values)
  ...
  simp only [Silean.Contracts.Cycle.CycleOutputRule.Holds, registeredRule,
...
@[simp] theorem memLaReadRule_holds_iff (inputs : inputMap.Values)
  ...
  simp only [Silean.Contracts.Cycle.CycleOutputRule.Holds, memLaReadRule,
    Silean.SignalGroup.fromLabels_matches_iff]
  constructor
...
@[simp] theorem memLaWriteRule_holds_iff (inputs : inputMap.Values)
  ...
  simp only [Silean.Contracts.Cycle.CycleOutputRule.Holds, memLaWriteRule,
    Silean.SignalGroup.fromLabels_matches_iff]
  constructor
...
@[simp] theorem memLaAddrRule_holds_iff (inputs : inputMap.Values)
  ...
  simp only [Silean.Contracts.Cycle.CycleOutputRule.Holds, memLaAddrRule,
    Silean.SignalGroup.fromLabels_matches_iff]
  constructor
...
@[simp] theorem memLaWdataRule_holds_iff (inputs : inputMap.Values)
  ...
  simp only [Silean.Contracts.Cycle.CycleOutputRule.Holds, memLaWdataRule,
    Silean.SignalGroup.fromLabels_matches_iff]
  constructor
...
@[simp] theorem memLaWstrbRule_holds_iff (inputs : inputMap.Values)
  ...
  simp only [Silean.Contracts.Cycle.CycleOutputRule.Holds, memLaWstrbRule,
    Silean.SignalGroup.fromLabels_matches_iff]
  constructor
...
@[simp] theorem memDoneRule_holds_iff (inputs : inputMap.Values)
  ...
  simp only [Silean.Contracts.Cycle.CycleOutputRule.Holds, memDoneRule,
    Silean.SignalGroup.fromLabels_matches_iff]
  constructor
...
@[simp] theorem memRdataWordRule_holds_iff (inputs : inputMap.Values)
  ...
  simp only [Silean.Contracts.Cycle.CycleOutputRule.Holds, memRdataWordRule,
    Silean.SignalGroup.fromLabels_matches_iff]
  constructor
...
@[simp] theorem memRdataLatchedRule_holds_iff (inputs : inputMap.Values)
  ...
  simp only [Silean.Contracts.Cycle.CycleOutputRule.Holds, memRdataLatchedRule,
    Silean.SignalGroup.fromLabels_matches_iff]
  constructor
...
@[simp] theorem memRdataQRule_holds_iff (inputs : inputMap.Values)
  ...
  simp only [Silean.Contracts.Cycle.CycleOutputRule.Holds, memRdataQRule,
    Silean.SignalGroup.fromLabels_matches_iff]
  constructor
