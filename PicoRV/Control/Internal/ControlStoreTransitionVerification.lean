...
module_child_certifications childContracts for body where
  ...
  notWdata := Silean.Primitives.notCertified.certification,
  halfOrWord := Silean.Modules.Mux.certification (.vector 2 .bit),
  storeSize := Silean.Modules.Mux.certification (.vector 2 .bit),
  startedWordsize := Silean.Modules.Mux.certification (.vector 2 .bit),
...
module_rule_schedules derivedRuleSchedules for body with childContracts
  ...
  output
    | .apply => [
        ...
        {.notDone, .notWdata, .notPrefetch} => Silean.Primitives.NotRule.apply,
        .waitForPrefetch => Silean.Primitives.AndRule.apply,
        {.halfOrWord, .storeSize, .startedWordsize} => Silean.Modules.Mux.Rule.select,
        ...
        .activeState => Silean.Modules.NamedTupleCombiner.Rule.apply,
...
theorem structuralTransition_eq_storeTransition (inputs : Inputs)
  ...
  cases prefetch : (current .mem_do_prefetch : Bool) <;>
    ...
    simp [activeState, selectedSize, Silean.SignalMap.set,
      prefetch, done, wdata, byte, half]
...
private theorem implements :
  ...
  have notWdataValue : hierStep.childOutputs .notWdata .output =
    ...
    have equation := (Silean.Primitives.notOutputRule_holds_iff _ _ _).mp
      ((childMatch .notWdata).ruleHolds Silean.Primitives.NotRule.apply)
    normalize_child_hyp equation unfolding wiring, context
