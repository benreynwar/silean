...
/-! Checked correspondence between the reader-facing Load-RS1 description and
its expanded typed production structure. -/

namespace PicoRV.Control.LoadRs1Transition.Description.Internal

open Silean Silean.Naming Silean.Authoring.CircuitDescription

private theorem same : some description = ofNaming LoadRs1Transition.naming := by
  simp only [circuit_description, description, construction]
  simp [circuit_description, enumeration]
  unfold LoadRs1Transition.moduleStructure LoadRs1Transition.naming
  simp only [id_eq]
  rw [ofNaming_composite]
  unfold ofCompositeNaming portList
  simp only [enumeration, List.map_cons, List.map_nil]
  dsimp [LoadRs1Transition.body, LoadRs1Transition.context,
    LoadRs1Transition.instancePorts, LoadRs1Transition.structuralChildren,
    LoadRs1Transition.wiring, sourceDescription, EndpointContext.moduleInput,
...
theorem corresponds : Corresponds description LoadRs1Transition.naming :=
  ⟨same, unique⟩

end PicoRV.Control.LoadRs1Transition.Description.Internal
