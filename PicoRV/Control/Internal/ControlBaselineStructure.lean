...
/-! Expanded typed structure for the reader-facing control baseline. -/
...
module_design Baseline (name := "picorv32_control_baseline") where
  ...
  instances {
    ...
    fetchCompleted (name := .indexed "and" 0) := Silean.Primitives.andDesign,
    result (name := .indexed "named_tuple_combiner" 0) :=
      Silean.Modules.NamedTupleCombiner.designWith
      stateMap ControlState.schema }
  named_wires {
    fetchCompleted := fetchCompleted.output }
  wiring {
