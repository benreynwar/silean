...
definition in `ControlTrapTransition.lean`. -/
...
module_design TrapTransition (name := "picorv32_control_trap_transition") where
  ...
  instances {
    updatedFields (name := .indexed "named_tuple_splitter" 0) :=
      Silean.Modules.NamedTupleSplitter.designWith
      stateMap ControlState.schema,
    trueBit (name := .indexed "constant" 0) :=
    ...
    resultState (name := .indexed "named_tuple_combiner" 0) :=
      ...
      stateMap ControlState.schema,
    falseBit (name := .indexed "constant" 1) :=
