...
module_design Alignment (name := "picorv32_control_alignment") where
  ...
  instances {
    ...
    sizeIsWord := Silean.Modules.EqualsConstant.design (.vector 2 .bit)
      (twoBitsOfNat 0),
    sizeIsHalf := Silean.Modules.EqualsConstant.design (.vector 2 .bit)
      (twoBitsOfNat 1),
    dataCommand := Silean.Primitives.orDesign,
    op1Low := Silean.Primitives.orDesign,
    pcLow := Silean.Primitives.orDesign,
    wordMisaligned := Silean.Primitives.andDesign,
    halfMisaligned := Silean.Primitives.andDesign,
    sizeMisaligned := Silean.Primitives.orDesign,
    dataMisaligned := Silean.Primitives.andDesign,
    instructionMisaligned := Silean.Primitives.andDesign }
  wiring {
  outputs {
