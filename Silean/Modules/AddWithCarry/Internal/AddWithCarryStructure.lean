...
namespace Silean.Modules.AddWithCarry.Naming

open Silean Silean.Naming

def naming : (width : Nat) → ModuleNaming (Modules.AddWithCarry.moduleStructure width)
  | 0 => by
      rw [Modules.AddWithCarry.moduleStructure.eq_def]
      exact .composite ⟨"add_with_carry", "base", []⟩ (ports 0)
        (fun | Modules.AddWithCarry.BaseInstance.empty => "empty")
        (fun
          | Modules.AddWithCarry.BaseInstance.empty =>
              Modules.Constant.Naming.naming (.vector 0 .bit)
                Modules.AddWithCarry.emptyValue)
  | width + 1 => by
      rw [Modules.AddWithCarry.moduleStructure.eq_def]
      exact .composite
        ⟨"add_with_carry", "ripple", [.natural (width + 1)]⟩
        (ports (width + 1))
        (fun
        ...
        (fun
          | .leftSplit | .rightSplit =>
              Silean.Naming.SignalAdapter.splitter
                (Modules.AddWithCarry.operandSplitter width)
          | .lowerLeft =>
              Silean.Naming.SignalAdapter.combiner
                (Modules.AddWithCarry.lowerCombiner width)
          | .lowerRight =>
              Silean.Naming.SignalAdapter.combiner
                (Modules.AddWithCarry.lowerCombiner width)
          | .lowerAdd => naming width
          | .highAdder => FullAdder.design.naming
          | .highBit => Silean.Naming.SignalAdapter.combiner
              Modules.AddWithCarry.highCombiner
          | .concat => VectorConcat.naming .bit width 1)

end Silean.Modules.AddWithCarry.Naming
