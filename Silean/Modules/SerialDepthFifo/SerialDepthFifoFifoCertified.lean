...
/-! Lifts the one-entry FIFO refinement through serial composition to prove the
abstract FIFO contract for every positive depth. No hardware is defined here. -/
...
private noncomputable def refinementFromAdditional (element : SignalType)
  ...
  | 0 => OneEntryFifo.cycleRefinement element
  | additionalDepth + 1 => by
      ...
      simpa [Nat.add_assoc, Nat.add_comm, Nat.add_left_comm] using
        Composition.FifoSerial.serialRefinement
          ...
          1 (additionalDepth + 1)
          (OneEntryFifo.cycleRefinement element)
...
noncomputable def fifoCertified (element : SignalType)
    ...
    Contracts.Fifo.FifoCertified (Silean.Interfaces.Fifo.ports element) (Silean.Interfaces.Fifo.payloadTypes element) := by
...
@[simp] theorem fifoCertified_moduleStructure (element : SignalType)
  ...
  simp [fifoCertified, fifoCertifiedFromAdditional, Contracts.Fifo.FifoCycleRefinement.certify,
    moduleStructure, additionalDepth_eq positive]
