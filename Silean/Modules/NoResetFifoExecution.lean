...
namespace Silean.Modules.NoResetFifo.Execution

open Silean.Contracts

abbrev Word (signalType : SignalType) := signalType.Denote
abbrev Input (signalType : SignalType) :=
  Silean.Contracts.NoResetFifo.Execution.Input (Word signalType)
...
def step (cycleBehavior : CycleBehavior signalType)
    ...
    Silean.Contracts.NoResetFifo.Execution.StepResult cycleBehavior.state.Values (Word signalType) :=
...
def model (cycleBehavior : CycleBehavior signalType) :
    Silean.Contracts.NoResetFifo.Execution.Model cycleBehavior.state.Values (Word signalType) where
...
theorem serial_step_decomposes (upstream downstream : CycleBehavior signalType)
    ...
    Silean.Contracts.NoResetFifo.Execution.StepDecomposition
...
def serial (upstream downstream : CycleBehavior signalType) :
    Silean.Contracts.NoResetFifo.Execution.Serial
...
end Silean.Modules.NoResetFifo.Execution
