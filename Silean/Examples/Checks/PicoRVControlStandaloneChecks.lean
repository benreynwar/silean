...
example : BitVector.toNat 8 (visibleOutputs .cpu_state) = cpuStateFetch := by decide
example : visibleOutputs .latched_store = true := by rfl
example : BitVector.toNat 5 (visibleOutputs .latched_rd) = 17 := by decide
example : BitVector.toNat 2 (visibleOutputs .mem_wordsize) = 2 := by decide
...
end Silean.Examples.Checks.PicoRVControlStandaloneChecks
