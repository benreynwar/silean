...
namespace Silean.Examples.Checks.FifoResetContract

open Silean
open Silean.Modules.Fifo
open Silean.Modules.Fifo.Reset

def inputs (valid data ready reset : Bool) :
    (Silean.Modules.Fifo.ports .bit).inputs.Values
  | .inputValid => valid
  | .inputData => data
  | .outputReady => ready
  | .reset => reset
...
def outputs (valid data ready : Bool) :
    (Silean.Modules.Fifo.ports .bit).outputs.Values
  | .outputValid => valid
  | .outputData => data
  | .inputReady => ready
...
private theorem matchesEmpty (data : Bool) :
    (Silean.Modules.Fifo.ports .bit).outputs.Matches
...
end Silean.Examples.Checks.FifoResetContract
