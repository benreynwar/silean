...
import Silean.Examples.PicoRV.Control.ControlNextTheorems

namespace Silean.Examples.Checks.PicoRVControlNextChecks
...
def decodedPhase (value : Nat) : PhaseDecode.outputMap.Values :=
  (PhaseDecode.cycleContract.evaluate
    (fun | .cpu_state => stateBits value) SignalMap.emptyValues).1
...
noncomputable example : Contracts.Cycle.ModuleCycleCertified ControlNext.ports :=
...
end Silean.Examples.Checks.PicoRVControlNextChecks
