...
namespace Silean.Examples.PicoRV.Decoder.InstructionFields
...
def funct3Layout (index : Fin 3) : Modules.VectorLayout.BitSource 32 :=
  .input ⟨index.val + 12, by omega⟩

def funct7Layout (index : Fin 7) : Modules.VectorLayout.BitSource 32 :=
  .input ⟨index.val + 25, by omega⟩

def opcodeLayout (index : Fin 7) : Modules.VectorLayout.BitSource 32 :=
  .input ⟨index.val, by omega⟩

def systemMiddleLayout (index : Fin 11) : Modules.VectorLayout.BitSource 32 :=
  .input ⟨index.val + 21, by omega⟩

def systemOuterLayout (index : Fin 13) : Modules.VectorLayout.BitSource 32 :=
...
module_design Structure (name := "PicoRVDecoderInstructionFields") where
  ...
  instances {
    funct3Bits := Modules.VectorLayout.design 32 3 funct3Layout,
    funct7Bits := Modules.VectorLayout.design 32 7 funct7Layout,
    opcodeBits := Modules.VectorLayout.design 32 7 opcodeLayout,
    systemMiddleBits := Modules.VectorLayout.design 32 11 systemMiddleLayout,
    systemOuterBits := Modules.VectorLayout.design 32 13 systemOuterLayout,
    funct3Equals (code : Fin 8 in Enumeration.fin 8)
      (name := s!"equals_funct3_{code.val}") :=
        Modules.EqualsConstant.design (.vector 3 .bit) (BitVector.ofNat 3 code.val),
    funct7Zero := Modules.EqualsConstant.design (.vector 7 .bit) (BitVector.ofNat 7 0),
    funct7Alternate := Modules.EqualsConstant.design (.vector 7 .bit)
      (BitVector.ofNat 7 0x20),
    opcodeSystem := Modules.EqualsConstant.design (.vector 7 .bit)
      (BitVector.ofNat 7 0x73),
    opcodeFence := Modules.EqualsConstant.design (.vector 7 .bit)
      (BitVector.ofNat 7 0x0f),
    systemMiddleZero := Modules.EqualsConstant.design (.vector 11 .bit)
      (BitVector.ofNat 11 0),
    systemOuterZero := Modules.EqualsConstant.design (.vector 13 .bit)
      (BitVector.ofNat 13 0),
    trueValue := Primitives.constantDesign true }
  wiring {
  outputs {
...
end Silean.Examples.PicoRV.Decoder.InstructionFields
