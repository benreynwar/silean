...
module_design Lookahead (name := "picorv32_memory_lookahead") where
  ...
  instances {
    ...
    enabledIdle := Primitives.andDesign,
    instructionCommand := Primitives.orDesign,
    readCommand := Primitives.orDesign,
    read := Primitives.andDesign,
    write := Primitives.andDesign,
    alignedNextPc := Modules.VectorLayout.design 32 32 Lookahead.alignedLayout,
    alignedRegOp1 := Modules.VectorLayout.design 32 32 Lookahead.alignedLayout,
    address := Modules.Mux.design (.vector 32 .bit),
    halfData := Modules.VectorLayout.design 32 32 Lookahead.halfDataLayout,
    byteData := Modules.VectorLayout.design 32 32 Lookahead.byteDataLayout,
    halfWordsize := Modules.EqualsConstant.design (.vector 2 .bit) (stateOfNat 1),
    byteWordsize := Modules.EqualsConstant.design (.vector 2 .bit) (stateOfNat 2),
    selectHalfData := Modules.Mux.design (.vector 32 .bit),
    selectByteData := Modules.Mux.design (.vector 32 .bit),
    lowAddress := Modules.VectorSlice.design .bit 0 2 30,
    highHalfBit := Modules.VectorSlice.design .bit 1 1 30,
    highHalf := Modules.EqualsConstant.design (.vector 1 .bit) (fun _ => true),
    lowHalfMask := Modules.Constant.design (.vector 4 .bit) (maskOfNat 0x3),
    highHalfMask := Modules.Constant.design (.vector 4 .bit) (maskOfNat 0xc),
    halfMask := Modules.Mux.design (.vector 4 .bit),
    byteMask := Modules.BinaryToOneHot.design 2,
    wordMask := Modules.Constant.design (.vector 4 .bit) (maskOfNat 0xf),
    selectHalfMask := Modules.Mux.design (.vector 4 .bit),
    selectByteMask := Modules.Mux.design (.vector 4 .bit) }
  wiring {
  outputs {
