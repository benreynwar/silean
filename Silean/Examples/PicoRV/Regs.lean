...
module_design Regs (name := "picorv32_regs") where
  ...
  instances {
    ...
    bank := Modules.RegisterBank.design Regs.wordType 5 2,
    -- Shared constants used to protect architectural register zero.
    zeroAddress := Modules.Constant.design Regs.addressType Regs.zeroAddressValue,
    zeroWord := Modules.Constant.design Regs.wordType Regs.zeroWordValue,
    -- Test all externally supplied register addresses against zero.
    rs1Zero := Modules.Equality.design Regs.addressType,
    rs2Zero := Modules.Equality.design Regs.addressType,
    rdZero := Modules.Equality.design Regs.addressType,
    -- Form resetn && cpuregs_write && (latched_rd != 0).
    rdNonzero := Primitives.notDesign,
    requestedWrite := Primitives.andDesign,
    enabledWrite := Primitives.andDesign,
    -- Select zero or the stored value for each architectural read.
    rs1Mux := Modules.Mux.design Regs.wordType,
    rs2Mux := Modules.Mux.design Regs.wordType }
  wiring {
    outputs {
