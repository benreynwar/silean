...
def outputValues (inputs : inputMap.Values) : outputMap.Values
  | .trap => decide (BitVector.toNat 8 (inputs .cpu_state) = cpuStateTrap)
  | .fetch => decide (BitVector.toNat 8 (inputs .cpu_state) = cpuStateFetch)
  | .loadRs1 => decide (BitVector.toNat 8 (inputs .cpu_state) = cpuStateLdRs1)
  | .loadRs2 => decide (BitVector.toNat 8 (inputs .cpu_state) = cpuStateLdRs2)
  | .execute => decide (BitVector.toNat 8 (inputs .cpu_state) = cpuStateExec)
  | .shift => decide (BitVector.toNat 8 (inputs .cpu_state) = cpuStateShift)
  | .store => decide (BitVector.toNat 8 (inputs .cpu_state) = cpuStateStmem)
  | .load => decide (BitVector.toNat 8 (inputs .cpu_state) = cpuStateLdmem)
