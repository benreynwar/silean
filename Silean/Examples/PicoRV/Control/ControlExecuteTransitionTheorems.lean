...
theorem outputs_of_allowed {step : cycleContract.Step}
    ...
    step.outputs .transition =
      ((fun inputs _ updated => executeTransition inputs updated) (Inputs.unpack (step.inputs .inputs))
        (stateMap.unpack (step.inputs .current))
        (stateMap.unpack (step.inputs .updated))).pack :=
  (PhaseTransition.outputRule_holds_iff (fun inputs _ updated => executeTransition inputs updated)
