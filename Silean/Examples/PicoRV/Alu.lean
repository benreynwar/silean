...
def wordSplitter : Composition.SignalSplitter := .vector 32 .bit
def wordCombiner : Composition.SignalCombiner := .vector 32 .bit
...
module_design Alu (name := "picorv32_alu") where
  ...
  instances {
    ...
    subtractMode := Primitives.orDesign,
    addSub := Modules.AddSub.design 32,
    -- Compute equality and bitwise candidates in parallel.
    equality := Modules.Equality.design Alu.wordType,
    bitwiseXor := Modules.BitwiseXor.design Alu.wordType,
    bitwiseOr := Modules.BitwiseOr.design Alu.wordType,
    bitwiseAnd := Modules.BitwiseAnd.design Alu.wordType,
    zeroBit := Modules.Constant.design .bit Alu.zeroBitValue,
    zeroWord := Modules.Constant.design Alu.wordType Alu.zeroWordValue,
    -- Derive comparison flags from equality, signs, and subtraction carry.
    unsignedLess := Primitives.notDesign,
    signDifference := Primitives.xorDesign,
    signedLess := Modules.Mux.design .bit,
    notEqual := Primitives.notDesign,
    notSignedLess := Primitives.notDesign,
    notUnsignedLess := Primitives.notDesign,
    -- Select comparison results in source priority order.
    selectUnsignedLess := Modules.Mux.design .bit,
    selectSignedLess := Modules.Mux.design .bit,
    selectUnsignedGreaterEqual := Modules.Mux.design .bit,
    selectSignedGreaterEqual := Modules.Mux.design .bit,
    selectNotEqual := Modules.Mux.design .bit,
    selectEqual := Modules.Mux.design .bit,
    ...
    xorSelected := Primitives.orDesign,
    orSelected := Primitives.orDesign,
    andSelected := Primitives.orDesign,
    selectAnd := Modules.Mux.design Alu.wordType,
    selectOr := Modules.Mux.design Alu.wordType,
    selectXor := Modules.Mux.design Alu.wordType,
    selectComparison := Modules.Mux.design Alu.wordType,
    selectArithmetic := Modules.Mux.design Alu.wordType }
  wiring {
    outputs {
