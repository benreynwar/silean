...
import Silean.Examples.PicoRV.Decoder.DecoderCaptureStageTheorems
...
private theorem implements :
  ...
  have resolveInputsEqual : ResolveStage.valuesOf
    ...
    simp [ResolveStage.valuesOf, resolveInputs, valuesOf, wiring, context,
      instancePorts, EndpointContext.moduleInput,
      EndpointContext.instanceOutput, SignalSource.value, captureOutputs]
  ...
  · constructor
    · change (layerChildren .capture).certification.stateCorresponds
        (captureState next)
          (HierStep.nextState (layerChildren .capture).moduleStructure
    ...
    · change (layerChildren .resolve).certification.stateCorresponds
        (resolveState next)
          (HierStep.nextState (layerChildren .resolve).moduleStructure
