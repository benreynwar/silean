...
module_design MemoryUpdateCore (name := "picorv32_datapath_memory_update") where
  ...
  instances {
    ...
    progress := Primitives.orDesign,
    active := Modules.Mux.design .bit,
    notActive := Primitives.notDesign,
    effectiveAddress := Modules.Add.design 32,
    effectiveOp1 := Modules.Mux.design (.vector 32 .bit),
    selectedOp1 := Modules.Mux.design (.vector 32 .bit),
    signedHalf := Modules.VectorLayout.design 32 32
      (MemoryUpdateCore.signExtendLayout 16),
    signedByte := Modules.VectorLayout.design 32 32
    ...
    selectUnsigned := Modules.Mux.design (.vector 32 .bit),
    loadDoneLeft := Primitives.andDesign,
    loadDone := Primitives.andDesign,
...
module_design StoreUpdate (name := "picorv32_datapath_store_update") where
  ...
  instances {
    isLoad := Modules.Constant.design .bit false,
...
module_design LoadUpdate (name := "picorv32_datapath_load_update") where
  boundary (StateUpdate.ports) (naming := StateUpdate.Naming.ports)
  instances {
    isLoad := Modules.Constant.design .bit true,
    update := MemoryUpdateCore.design }
  wiring {
  outputs { .state := update.state }
  instance (.isLoad) {}
