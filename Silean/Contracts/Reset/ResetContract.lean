...
/-! # Reset-synchronized trace contracts

A `ModuleResetContract` describes the expected outputs and specification-state
transition on every cycle after a synchronous reset. Before the first reset,
the contract deliberately places no requirements on behavior, so correctness
does not depend on an unknown hardware initial state.

Its state may be any Lean type. Unlike a cycle contract, certification does not
provide a cycle-by-cycle mapping to structural state. Instead it proves that
every structural execution trace following reset matches the specification
trace. Output expectations may require zero or one, or mark a bit as
`dontCare`. -/

structure ModuleResetContract (ports : ModulePorts) where
  /-- Arbitrary Lean state used by the behavioral specification. -/
  State : Type u
  ...
  /-- Specification state established by an asserted reset edge. -/
  resetState : State
  /-- Expected outputs and next specification state for one cycle. -/
