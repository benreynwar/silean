...
namespace PicoRVTests.ControlCommandFinishChecks
...
def result (clear : Bool) (transition : Transition) : stateMap.Values :=
  stateMap.unpack (((CommandFinish.cycleContract.evaluate
    (inputs clear transition) Silean.SignalMap.emptyValues).1) .state)
...
noncomputable example : Silean.Contracts.Cycle.ModuleCycleCertified CommandFinish.ports :=
...
end PicoRVTests.ControlCommandFinishChecks
