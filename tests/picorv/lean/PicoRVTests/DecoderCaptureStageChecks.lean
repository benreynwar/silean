...
namespace PicoRVTests.DecoderCaptureStageChecks

open Silean
open Silean.FIRRTL
open PicoRV.Decoder.CaptureStage
...
def capture (word : Nat) (resetn := true) : Inputs where
  ...
  mem_do_rinst := true
  mem_done := true
  mem_rdata_latched := PicoRV.Decoder.wordOfNat word

def idle : Inputs where
  resetn := true
  mem_do_rinst := false
  mem_done := false
  mem_rdata_latched := PicoRV.Decoder.wordOfNat 0
...
example : Silean.BitVector.toNat 5 (addiState .decoded_rd) = 1 := by decide
example : Silean.BitVector.toNat 5 (addiState .decoded_rs1) = 2 := by decide
...
example : Silean.BitVector.toNat 5 (resetBranch .decoded_rd) = 8 := by decide
...
example : PicoRV.Decoder.structuralChildren .capture = moduleStructure := rfl
example : PicoRV.Decoder.structuralChildren .resolve =
    PicoRV.Decoder.ResolveStage.moduleStructure := rfl

end PicoRVTests.DecoderCaptureStageChecks
