...
namespace SileanTests.AddSubWithCarry
...
def inputs (width : Nat) (left right : Fin width → Bool) (subtract : Bool) :
    (Modules.AddSubWithCarry.ports width).inputs.Values
  | .left => left
  | .right => right
  | .subtract => subtract

def outputs (width : Nat) (left right : Fin width → Bool) (subtract : Bool) :=
  ((Modules.AddSubWithCarry.cycleContract width).evaluate
    (inputs width left right subtract) SignalMap.emptyValues).1

noncomputable example : Contracts.Cycle.ModuleCycleCertified
    (Modules.AddSubWithCarry.ports 4) :=
  Modules.AddSubWithCarry.certified 4

example : Contracts.Cycle.Implements
    (Modules.AddSubWithCarry.moduleStructure 4)
    (Modules.AddSubWithCarry.cycleContract 4)
    (Modules.AddSubWithCarry.certification 4).stateCorresponds :=
  Modules.AddSubWithCarry.implements_contract 4
...
example (width : Nat) (left right : Fin width → Bool) (subtract : Bool) :
  ...
  simpa [outputs, inputs, Contracts.Cycle.ModuleCycleContract.evaluateStep] using
    Modules.AddSubWithCarry.result_toNat_of_allowed width
      ((Modules.AddSubWithCarry.cycleContract width).evaluateStep_allowed
...
example (width : Nat) (left right : Fin width → Bool) :
  ...
  have equation := Modules.AddSubWithCarry.cycleContract.carryOut width
    ((Modules.AddSubWithCarry.cycleContract width).evaluateStep_allowed
      (inputs width left right true) SignalMap.emptyValues)
  change @Eq Bool
    (((Modules.AddSubWithCarry.cycleContract width).evaluate
      (inputs width left right true) SignalMap.emptyValues).1 .carryOut)
    (Modules.AddSubWithCarry.addSubBits width left right true).2 at equation
  exact equation.trans
    (Modules.AddSubWithCarry.addSubBits_carry_subtract width left right)

private def contains (text fragment : String) : Bool :=
  (text.splitOn fragment).length > 1

#guard match renderRootModule (Modules.AddSubWithCarry.naming 4) with
  | .error _ => false
  | .ok text =>
      ["public module AddSubWithCarry_4", "input left : UInt<1>[4]",
       ...
       "inst add_with_carry_0 of add_with_carry_ripple_4",
       "connect bitwise_xor_0.right, combiner_0.aggregate_0",
       "connect add_with_carry_0.right, transformedRight",
       "connect add_with_carry_0.carry_in, subtract"].all (contains text)

end SileanTests.AddSubWithCarry
