...
namespace SileanTests.AddWithCarry

open Silean Silean.FIRRTL

noncomputable example : Contracts.Cycle.ModuleCycleCertified
    (Modules.AddWithCarry.ports 0) :=
  Modules.AddWithCarry.certified 0
noncomputable example : Contracts.Cycle.ModuleCycleCertified
    (Modules.AddWithCarry.ports 1) :=
  Modules.AddWithCarry.certified 1
noncomputable example : Contracts.Cycle.ModuleCycleCertified
    (Modules.AddWithCarry.ports 4) :=
  Modules.AddWithCarry.certified 4
...
def inputs (width : Nat) (left right : Fin width → Bool) (carryIn : Bool) :
    (Modules.AddWithCarry.ports width).inputs.Values
  | .left => left
  | .right => right
  | .carryIn => carryIn

def outputs (width : Nat) (left right : Fin width → Bool) (carryIn : Bool) :=
  ((Modules.AddWithCarry.cycleContract width).evaluate
...
example (width : Nat) (left right : Fin width → Bool) (carryIn : Bool) :
  ...
  exact Modules.AddWithCarry.numeric_value_of_allowed width
    ((Modules.AddWithCarry.cycleContract width).evaluateStep_allowed
      (inputs width left right carryIn) SignalMap.emptyValues)

private def contains (text fragment : String) : Bool :=
  (text.splitOn fragment).length > 1

#guard match renderRootModule (Modules.AddWithCarry.Naming.naming 0) with
  | .error _ => false
  | .ok text =>
      ["public module add_with_carry_base", "input left : UInt<1>[0]",
...
#guard match renderRootModule (Modules.AddWithCarry.Naming.naming 3) with
  | .error _ => false
  | .ok text =>
      ["public module add_with_carry_ripple_3", "inst left_split", "inst right_split",
       "inst add_lower of add_with_carry_ripple_2",
