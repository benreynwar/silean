...
async def reads_current_entry_and_writes_one_selected_entry(dut) -> None:
    ...
    set_address(dut, "read_0_address", 0)
    ...
    for address, expected in enumerate(expected_values):
        set_address(dut, "read_0_address", address)
        await FallingEdge(dut.clock)
        assert int(dut.read_0_value.value) == expected

    # A same-address read observes the old value before the active edge.
    set_address(dut, "read_0_address", 1)
    set_address(dut, "write_address", 1)
    dut.write_value.value = 0
    dut.write_enable.value = 1
    await ReadOnly()
    assert int(dut.read_0_value.value) == 1

    await next_drive_phase(dut.clock)
    assert int(dut.read_0_value.value) == 0

    # Other entries retain their values.
    dut.write_enable.value = 0
    set_address(dut, "read_0_address", 2)
    await FallingEdge(dut.clock)
    assert int(dut.read_0_value.value) == 0
